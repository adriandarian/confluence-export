...
def convert_confluence_to_markdown(html_content: str) -> str:
    ...
    def replace_code_macro(match):
        ...
        code_match = re.search(r'<ac:plain-text-body[^>]*><!\[CDATA\[(.*?)\]\]></ac:plain-text-body>', full_match, re.DOTALL)
        if not code_match:
            code_match = re.search(r'<ac:plain-text-body[^>]*>(.*?)</ac:plain-text-body>', full_match, re.DOTALL)
    ...
    def replace_link(match):
        ...
        if page_match:
            ...
            # Get link text
            link_text_match = re.search(r'<ac:(?:plain-text-)?link-body[^>]*>([^<]+)</ac:', full_match)
